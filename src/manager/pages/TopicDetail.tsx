        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {/* Type badge */}
              <span className="text-xs px-2 py-0.5 bg-gray-100 rounded">{topic.type}</span>

              {/* Status dropdown */}
              <select
                value={topic.status}
                onChange={(e) => handleStatusChange(e.target.value as TopicStatus)}
                className={`text-xs px-2 py-0.5 rounded border-0 cursor-pointer focus:ring-2 focus:ring-blue-500 ${
                  topic.status === 'active'
